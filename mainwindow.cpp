...
ui->lineEdit_price->setValidator(new QIntValidator(0,9999,this));//cs
     ...
     int ret=A.connect_arduino(); // lancer la connexion à arduino
               ...
               connect(ui->arduino, &QPushButton::clicked, this, &ArtNexus::on_arduinoButtonClicked);
                   connect(ui->arduino1, &QPushButton::clicked, this, &ArtNexus::on_arduinoButtonClicked);
                   connect(ui->sortComboBox, QOverload<int>::of(&QComboBox::activated), this, &ArtNexus::sortData);
...
void ArtNexus::on_pushButton_19_clicked()
{
    ...
    filePath = QDir(filePath).filePath("C:/Users/medal/Downloads/ardIntegf/integr ard_qt/int_event - Copie (2)/int_event - Copie (2)/listeOeuvres.pdf"); // Chemin du fichier
...
void ArtNexus::on_arduinoButtonClicked() {
...
void ArtNexus::on_stats_2_clicked()
{QSqlQuery query;
    ...
    if (query.exec())
    {
        ...
        Dialog *dialog = new class Dialog(this);
        dialog->setChartDataassoc(percentagePerDomain);
        dialog->exec();
    }
}

void ArtNexus::on_lesearch_2_textChanged(const QString &arg1)
{
    QString searchStr = ui->lesearch_2->text().trimmed();
    if (!searchStr.isEmpty()) {
        QString filter = QString("id_association LIKE '%%1%' OR nom_association LIKE '%%1%' OR adresse_association LIKE '%%1%' OR tel_association LIKE '%%1%' OR domaine_association LIKE '%%1%' OR montant_association LIKE '%%1%' ") .arg(searchStr);
        QSqlQueryModel *filteredModel = new QSqlQueryModel();
        filteredModel->setQuery("SELECT id_association, nom_association, adresse_association,  tel_association, domaine_association,montant_association FROM ASSOCIATIONS WHERE " + filter);
...
void ArtNexus::sortData(int index) {
    QString columnName = ui->sortComboBox->itemText(index);

    ui->tableView_Association->setModel(Ctemp.tri(columnName));
}

void ArtNexus::on_employees_clicked()
{
    ui->stackedWidget_5->setCurrentIndex(0);
    ui->tableView_emp->setModel(Etmp.afficher());
}

void ArtNexus::on_pushButton_6_clicked()
{
    ui->stackedWidget_3->setCurrentIndex(0);
    ui->tableView_Association->setModel(Ctemp.afficher());
}
